...
# CANCEL: 5) a new posibility algorithm focus on killer number
        #    (NOTE: current posibility model is better)
         ...
         # by police,once he is dead,game over and killer lost.
...
def game():
    ...
    godfather=player_ID[len(player_ID)-1]
    checklist=[]
    lostclue=False
    ...
    while (not game_end):
        ...
        ###################################
        # police checking
        if(set(player_ID)==set(checklist)):
                checklist=[]
                for i in policeID:
                    checklist.append(i)
                lostclue=True  #police lost clue about killer/godfather
        ...
        if (player_ID[check] in killers) and (player_ID[check]!=godfather):
            find_killer=True
        else:
            find_killer=False
            checklist.append(player_ID[check])
            # police check all people while cannot figure out the
            # godfather
            if(set(player_ID)==set(checklist)):
                checklist=[]
                for i in policeID:
        ...
        if find_killer:
            ...
            for i in range(nt):
                ...
                if player_ID[i] in policeID:
                    if lostclue:
                        vote[i]=vc*nc+vs+vk*n_killer
                    else:
                        vote[i]=vc*nc+vs*n_police/nil+vk*n_killer
                if (player_ID[i] in killers):
                    vote[i]=vc*nc+vs+vp*n_police
                #update godfather
                if player_ID[i]==godfather:
                    if godfather in checklist:
                        vote[i]=vc*nc+vs*n_police/nil+vs
            # if police/killer number larger/equal to half total
            # number,not vote needed
            if n_police>=0.5*len(player_ID):
                killer_win=False
                break
            if n_killer>=0.5*len(player_ID):
                killer_win=True
                break
            # voting
            # evenly voting
            ...
            if player_ID[voted] in killers:
                ...
                n_killer-=1
            if n_killer==0 or player_ID[voted]==godfather:
                #if godfather is out,game end immediately
                game_end=True
                killer_win=False
                break
